        ...
        if (!this.data) {
            ...
            for (const face of this.faces) {
                for (const facePoint of face) {
                    ...
                    if (typeof k === "number") {
                        ...
                        if (
                            typeof facePoint.u === "number" &&
                            typeof facePoint.v === "number"
                        ) {
            ...
            this.data = {
                attributes: {
                    ...
                    normal: createFloat32ArrayFromVector3Array(
                        normals.map(item => (item ? item : [0, 0, 0]))
                    ),
...
function computeNormals(elements: number[], points: Vector3[]) {
    const normals: Vector3[] = new Array(points.length).map(() => [0, 0, 0])
