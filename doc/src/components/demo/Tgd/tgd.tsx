...
interface TgdProps<T extends SettingsDefinitions> {
    ...
    onReady(scene: TgdContext, assets: Assets): void | undefined | ((settings: Record<keyof T, number>) => void)
    width?: string
    height?: string
    noBorder?: boolean
    gizmo?: boolean
...
export default function Tgd<T extends SettingsDefinitions>({
    ...
    const reduceSettings = React.useCallback((): Record<keyof T, number> => {
    ...
    if (width.endsWith("px") && height.endsWith("px")) {
        aspectRatio = `${width.substring(width.length - 2)} / ${height.substring(height.length - 2)}`
    }
    ...
    React.useEffect(() => {
        ...
        if (!canvas.requestFullscreen) {
            canvas.requestFullscreen = (canvas as unknown as Record<string, unknown>)["webkitRequestFullscreen"] as (
    ...
    if (noBorder) {
        return (
            <div
                ...
                {gizmo && <canvas className={styles.gizmo} ref={mountGizmo} />}
    ...
    return (
        <div className={Theme.classNames.join(className, styles.Tgd)}>
            ...
            <div className={styles.relative}>
                ...
                <div className={Theme.classNames.join(styles.overlay, loading && styles.loading)}>
                    <Spinner />
                </div>
                {children}
            </div>
...
async function loadAssets({ glb, data, text, image }: Partial<AssetsToLoad> = {}): Promise<Assets> {
    const assets: Assets = {
        glb: {},
        data: {},
        text: {},
