...
function SingleCodeViewerView(props: SingleCodeViewerViewProps) {
    ...
    React.useEffect(() => {
        window.setTimeout(() => {
            ...
            const html = Prism.highlight(props.value, getGrammarForLanguage(props.language), props.language)
            code.innerHTML = html
        }, 100)
    ...
    return (
        <div className={getClassNames(props)} onDoubleClick={handleDoubleClick} id={props.id}>
            <pre>
                <code ref={refCode} className={`language-${props.language}`} />
            </pre>
            {popup && (
                <button className={Style.popup} onClick={handlePopupClick} type="button">
    ...
    return (
        <>
            {captions.map((caption, index) => (
                <details key={caption} open={false} id={props.id}>
                    <summary>{caption}</summary>
                    <SingleCodeViewerView value={props.value[caption]} language={props.language} />
                </details>
            ))}
        </>
    )
