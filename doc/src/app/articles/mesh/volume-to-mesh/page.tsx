...
import { tgdCalcClamp, tgdDataMarchingCubesConfigurations } from "@tolokoban/tgd"
...
export default function Page() {
    const [caseNumber, setCaseNumber] = useLocalStorageState(0, "volume-to-mesh/caseNumber", (value) =>
        tgdCalcClamp(Math.floor(ensureNumber(value, 0)), 0, 255),
    )
    const [triangles, setTriangles] = React.useState("")
    const key = `volume-to-mesh/triangles/${caseNumber}`
    React.useEffect(() => {
        setTriangles(globalThis.localStorage.getItem(key) ?? CONFIGURATIONS[caseNumber])
    ...
    return (
        <div>
            ...
            <p>
                <a href="https://en.wikipedia.org/wiki/Marching_cubes">Marching cubes</a> is a technique to generate
                meshes from a volume.
                <br />A volume is a function that tells you, for each point in a box, it you are inside or outside an
                object.
            </p>
            <p>
                A voxel is a unit cube. Each corner of a voxel is either inside the volume (<b>red</b>) or outside (
                <b>blue</b>). With triangles, touching the edges of the voxel, we need to separate the space between red
                and blue corners.
            </p>
            ...
            <ViewSlider
                ...
                step={1}></ViewSlider>
            <br />
            <ViewPanel display="flex" justifyContent="space-between" alignItems="center">
                <ViewPanel display="flex" alignItems="center" fullwidth gap="M">
                    ...
                    <ViewFloatingButton icon={IconArrowRight} onClick={handleRight} />
                </ViewPanel>
