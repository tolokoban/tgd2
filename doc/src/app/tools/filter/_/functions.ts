...
export function useGlslFunctions(): Record<string, () => TgdCodeFunctions> {
    const [functions, setFunctions] = React.useState<Record<string, () => TgdCodeFunctions>>({})
        ...
        import("@tolokoban/tgd").then((module) => {
            ...
            for (const key of Object.keys(module)) {
                if (key.startsWith(prefix)) {
                    result[key.slice(prefix.length)] = module[key as keyof typeof module] as () => TgdCodeFunctions
                }
            }
            setFunctions(result)
        })
