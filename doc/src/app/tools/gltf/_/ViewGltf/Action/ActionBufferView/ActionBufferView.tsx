...
export function ViewActionBufferView({ className, data, index }: ViewActionBufferViewProps): React.ReactNode {
    ...
    const lines = range(Math.min(MAX_LINES, Math.ceil((view.length - start) / columns)))

    return (
        <ViewStrip className={$.join(className, Styles.actionBufferView)} template="1*" orientation="column">
            <ViewPanel display="flex" flexDirection="column">
                <ViewHeader index={index} bufferView={bufferView} />
                <ViewPanel color="primary-4" padding="S" display="flex" justifyContent="flex-start" gap="XL">
                ...
                {max > MAX_LINES * columns - 1 && (
                    <ViewPanel color="primary-3" padding="S">
                        <ViewSlider wide min={0} max={max} step={columns} value={start} onChange={setStart} />
                    </ViewPanel>
                )}
            </ViewPanel>
                ...
                {lines.map((line) => (
                    <>
                        ...
                        {range(columns).map((col) => (
                            <div key={`line/${line}/${col}`}>{view[start + line * columns + col]}</div>
                        ))}
                    </>
                ))}
            </div>
