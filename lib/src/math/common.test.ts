export function expectVectorsToBeClose(v1: ArrayLike<number>, v2: ArrayLike<number>, precision = 5) {
    try {
        ...
        for (let index = 0; index < v1.length; index++) {
            ...
            if (d > Math.pow(10, -precision)) throw new Error(`${v1[index]} is not close to ${v2[index]}!`)
        }
    } catch (error) {
