import { padColOfNumbers, TgdConsole } from "@tgd/debug"
import { mat4 } from "gl-matrix"
import { ArrayNumber3, ArrayNumber4, ArrayNumber16 } from ".."
...
export class TgdMat4 extends Float32Array {
	...
	transpose(): this {
		// prettier-ignore
		const { m01, m02, m03, m10, m12, m13, m20, m21, m23, m30, m31, m32 } = this
		this.m01 = m10
		this.m02 = m20
		this.m03 = m30
		this.m10 = m01
	...
	debug(caption = "Mat4") {
		...
		for (let i = 0; i < values.length; i++) {
			...
			if (value.trim().toLowerCase() === "nan") {
				C.add(value, { bold: true, color: "#f30" })
			} else {
				C.add(value)
			}
			if (i % 4 === 3) {
