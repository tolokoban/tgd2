...
export class TgdMaterial {
    ...
    get fragmentShaderCode(): TgdCodeBloc {
        if (typeof this._fragmentShaderCode === "function") return this._fragmentShaderCode()
        return this._fragmentShaderCode
    }

    get fragmentShaderHeader(): TgdCodeBloc {
        if (typeof this._fragmentShaderHeader === "function") return this._fragmentShaderHeader()
        return this._fragmentShaderHeader
    }

    private readonly _extraFragmentShaderFunctions: TgdCodeFunctions | (() => TgdCodeFunctions)

    get extraFragmentShaderFunctions() {
        if (typeof this._extraFragmentShaderFunctions === "function") return this._extraFragmentShaderFunctions()
    ...
    get vertexShaderCode() {
        if (typeof this._vertexShaderCode === "function") return this._vertexShaderCode()
        return this._vertexShaderCode
    }

    private readonly _extraVertexShaderFunctions: TgdCodeFunctions | (() => TgdCodeFunctions)

    get extraVertexShaderFunctions() {
        if (typeof this._extraVertexShaderFunctions === "function") return this._extraVertexShaderFunctions()
        return this._extraVertexShaderFunctions
    }

    private readonly _vertexShaderCodeForGetPosition?: TgdCodeBloc | (() => TgdCodeBloc)
    ...
    get vertexShaderCodeForGetPosition() {
        if (typeof this._vertexShaderCodeForGetPosition === "function") return this._vertexShaderCodeForGetPosition()
        return this._vertexShaderCodeForGetPosition
    }

    readonly setUniforms: undefined | ((materialContext: TgdMaterialContext) => void)

    protected readonly state: Partial<TgdPainterStateOptions>

    public applyState(context: { gl: WebGL2RenderingContext; webglParams: WebglParams }, action: () => void) {
