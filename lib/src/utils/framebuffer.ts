...
export function webglCreateFramebuffer(gl: WebGL2RenderingContext): WebGLFramebuffer {
...
export function webglRenderbufferStorageMultisample(
    ...
    if (error !== gl.NO_ERROR) {
        console.error(
            `WebGL Error in renderbufferStorageMultisample: ${webglLookup(error)} (${error})\n
samples:        ${samples}
...
height:         ${height}`,
        ...
        if (!availableSamples.includes(samples)) {
            console.error(
                `Available samples for format gl.${webglLookup(internalFormat)} are ${JSON.stringify(
                    availableSamples,
