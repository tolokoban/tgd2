...
export const webglPresetCull: Record<"off" | "back" | "front", WebglCullOptions> = {
    off: {
        enabled: false,
...
export function webglCullSet(context: { webglParams: WebglParams }, cull: WebglCullOptions) {
    const p = context.webglParams
    p.cullFace = cull.enabled
    p.cullFaceMode = cull.cullFace
}

export function webglCullGet(context: { webglParams: WebglParams }): WebglCullOptions {
    const p = context.webglParams
    return {
        enabled: p.cullFace,
...
export function webglCullExec(context: { webglParams: WebglParams }, cull: WebglCullOptions, action: () => void) {
