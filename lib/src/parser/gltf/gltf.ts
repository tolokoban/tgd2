...
export class TgdDataGlb {
	...
	setAttrib(
		...
		if (!isNumber(accessorIndex)) {
			console.log("🚀 [gltf] accessorIndex =", accessorIndex) // @FIXME: Remove this line written on 2025-07-07 at 16:00
			throw new Error(
				`Attribute is not an accessor index, but this: ${JSON.stringify(accessorIndex)}`,
			)
		}
