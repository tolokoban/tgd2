...
import { type TgdBoundingBox, TgdConsole } from ".."
...
export abstract class TgdCamera implements TgdInterfaceTransformable {
    ...
    setCurrentState(state: Readonly<TgdCameraState>): this {
     ...
     */
    fitSpaceAtTarget(width: number, height: number, cover = false): this {
    ...
    /**
     * Adapt the camera position, distance, near and far planes to
     * allow easy orbitting around a bounding box.
    ...
    fitBoundingBox(bbox: TgdBoundingBox): this {
        ...
        this.near = 0.1
        this.transfo.distance = radius + 2 * this.near
        this.transfo.setOrientation([0, 0, 0, 1])
        this.zoom = 1
        this.fitSpaceAtTarget(width * 0.5, height * 0.5)
        this.far = this.near + this.transfo.distance * 2
        return this
    }
    ...
    /**
     * Transform a point from space coordinates to screen coordinates.
     */
    ...
    debug(caption?: string) {
        const C = new TgdConsole()
        const name = `${this.name}: ${caption ?? ""}`
        C.add("TgdCamera: ").add(name, { bold: true }).nl()
        C.add(`    Near/Far: ${this.near} / ${this.far}`).nl()
        C.add(`    Distance: ${this.transfo.distance}`).nl()
        C.add(`    Zoom: ${this.zoom}`).nl()
        C.debug()
