...
import {
	TgdDataset,
	type TgdDatasetOptions,
	type TgdDatasetType,
	type TgdDatasetTypeRecord,
} from "@tgd/dataset"
...
import { TgdPainterLogic, TgdVertexArray } from "@tolokoban/tgd"
import {
	TgdUniformBufferObject,
	type TgdUniformBufferObjectOptions,
} from "./../uniform/uniform-buffer-object"
import { TgdPainter } from "./painter"
import { TgdPainterState, type TgdPainterStateOptions } from "./state"

interface DatasetOptions
	extends Partial<Omit<TgdDatasetOptions, "data" | "buffer">> {
	attribs: Record<
		string,
		{
			type: TgdDatasetType
...
export interface TgdPainterProgramOptions {
	...
	uniforms?: Record<
		string,
		TgdUniformBufferObject | Omit<TgdUniformBufferObjectOptions, "bindingPoint">
	>
	...
	state?: Omit<TgdPainterStateOptions, "children">
...
export class TgdPainterProgram extends TgdPainter {
	...
	constructor(
		...
		this.drawMode =
			typeof drawMode === "number" ? drawMode : context.gl[drawMode]
		...
		this.elementsCount =
			elementsCount ?? resolveElementsCount(datasets, elements)
		...
		Object.keys(uniforms).forEach((blockName, index) => {
			...
			if (setBindingPoints.has(block.bindingPoint)) {
				const code: TgdCodeBloc = Object.keys(uniforms).map(
					(blockName, index) =>
	...
	private uniformsToCode(
		uniforms: Record<
			string,
			| TgdUniformBufferObject
			| Omit<TgdUniformBufferObjectOptions, "bindingPoint">
		>,
	) {
		...
		const reservedBindingPoints = new Set<number>()
		for (const blockName of Object.keys(uniforms)) {
			const block = this.resolveUniformBlock(
				uniforms[blockName],
				reservedBindingPoints,
			)
	...
	private resolveUniformBlock(
		obj:
			| TgdUniformBufferObject
			| Omit<TgdUniformBufferObjectOptions, "bindingPoint">,
		reservedBindingPoints: Set<number>,
	): TgdUniformBufferObject {
		if (obj instanceof TgdUniformBufferObject) {
			reservedBindingPoints.add(obj.bindingPoint)
			return obj
		}

		for (let bindingPoint = 0; bindingPoint < 255; bindingPoint++) {
			if (reservedBindingPoints.has(bindingPoint)) continue

			reservedBindingPoints.add(bindingPoint)
			const block = new TgdUniformBufferObject(this.context, {
				...obj,
				bindingPoint,
			})
			this.uniformBlocksToDelete.push(block)
			return block
		}
		throw new Error(
			"[TgdPainterProgram] There are too many uniform buffer objects!",
		)
	}
}

function resolveAttributesFromDataset(
	datasets: TgdDataset[],
): TgdCodeVariables<WebglAttributeType> | undefined {
...
function resolveElementsCount(
	datasets: TgdDataset[],
	elements: TgdTypeArrayForElements | undefined,
): number {
...
function resolveInstancesCount(datasets: TgdDataset[]): number {
	const counts = datasets
		.filter((ds) => ds.divisor > 0)
		.map((ds) => ds.count / ds.divisor)
	...
	throw new Error(
		`[TgdPainterPogram] I cannot resolve elementsCount because your instance datasets have different counts!\n${datasets
			...
			.map(
				(ds) =>
					`  ${ds.name}: count == ${ds.count} / ${ds.divisor} == ${ds.count / ds.divisor}`,
			)}`,
	)
}
