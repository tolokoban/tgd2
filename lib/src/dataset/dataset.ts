...
export class TgdDataset {
    ...
    getType(attribName: string) {
        const { attributesDefinition } = this
        const type = attributesDefinition[attribName]
        if (!type)
            throw new Error(
                `[TgdDataset.getType] Attribute name not found in this dataset: "${
                    attribName
    ...
    assertAttribType(attribName: string, ...types: string[]): this {
        ...
        if (!type)
            throw new Error(
                `Attribute "${attribName}" does not exist! Available names are: ${Object.keys(this.attributesDefinition)
                    .sort()
                    .join(", ")}.`,
            )
    ...
    set(
        ...
        this.count = Math.max(
            this.count,
            Math.min(count, Math.floor((buffer.byteLength - sourceOffset) / sourceStride)),
        )
    ...
    debug(caption = "Dataset") {
        ...
        const rows: [name: string, type: string, offset: string][] = [["Name", "type", "offset"]]
        ...
        const sizes: number[] = [0, 1, 2].map((index) =>
            rows.reduce((previous, current) => Math.max(previous, current[index].length), 0),
        )
