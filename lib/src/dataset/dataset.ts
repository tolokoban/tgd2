...
export class TgdDataset {
		...
		if (!type)
			throw new Error(
				`[TgdDataset.getType] Attribute name not found in this dataset: "${
					attribName
				}"!\nAvailable attributes names are: ${Object.keys(
	...
	assertAttribType(attribName: string, ...types: string[]): this {
		...
		if (!types.includes(type)) {
			throw new Error(
				`Attribute "${attribName}" is of type "${type}", which is not ${types.join(" nor ")}!`,
			)
		}
		return this
	}
	...
	debug(caption = "Dataset") {
		...
		const sizes: number[] = [0, 1, 2].map((index) =>
			rows.reduce(
				(previous, current) => Math.max(previous, current[index].length),
				0,
			),
		)
...
const resize =
	typeof ArrayBuffer.prototype.transfer === "function" ? resizeFast : resizeSlow
