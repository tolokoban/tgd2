...
export class TgdBoundingBox {
    static from({ min, max }: { min: ArrayNumber3 | ArrayNumber4 | TgdVec3 | TgdVec4, max: ArrayNumber3 | ArrayNumber4 | TgdVec3 | TgdVec4 }) {
        const [x0, y0, z0] = min
        const [x1, y1, z1] = max
        const bbox = new TgdBoundingBox()
        bbox.addPoint(x0, y0, z0)
        bbox.addPoint(x1, y1, z1)
        return bbox
    }
