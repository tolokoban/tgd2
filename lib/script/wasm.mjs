 ...
 * @types {string[]}
 */
const output = ["/* eslint-disable unicorn/prevent-abbreviations */", `import { TgdCodeBloc } from "@tgd/shader"`, ""]
...
for (const [name, type, args] of lines) {
    ...
    if (startsWith(type, "CONST")) {
        output.push(`export function ${jsName}(value: number): ${T} {`, `    return [\`(${name} \${value})\`]`, "}")
        continue
    }
    if (startsWith(type, "UNARY", "CONVERT")) {
        output.push(`export function ${jsName}(value: ${T}): ${T} {`, `    return ["(${name}", [...value], ")"]`, "}")
        continue
    }
